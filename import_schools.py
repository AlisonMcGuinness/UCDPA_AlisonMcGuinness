...
def get_schools_data():
    ...
    for year, year_info in school_years.items():
        year = import_classes_by_year(school_types, year, year_info)
...
def import_classes_by_year(schools, year, year_info):
    ...
    # the file name is based on the year
    ...
    # make a list of what cols we want to import
    # we only need the column with the school roll number (0)
    # and all the cols from Class 1 to Class 40 (depending on file)
    keep_cols = [x for x in range(year_info["start"], year_info["start"] + year_info["cols"])]
    keep_cols.append(year_info["roll"])

    # Read the right cols from the right sheet from the excel file for this year
    ...
    # add year col to mark all the data from this file with the correct year
    all_data['year'] = year
